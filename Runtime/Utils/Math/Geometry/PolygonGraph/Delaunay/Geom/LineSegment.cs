...
using LBF.Math.Geometry.PolygonGraph.Delaunay.Delaunay;

namespace LBF.Math.Geometry.PolygonGraph.Delaunay.Geom {

	public class LineSegment {

		public static List<LineSegment> VisibleLineSegments(List<Delaunay.Edge> edges) {
			List<LineSegment> segments = new List<LineSegment>();

			foreach (Delaunay.Edge edge in edges) {
				if (edge.Visible()) {
