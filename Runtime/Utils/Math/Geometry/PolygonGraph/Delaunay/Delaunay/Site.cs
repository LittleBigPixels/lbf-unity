...
using LBF.Math.Geometry.PolygonGraph.Delaunay.Geom;

namespace LBF.Math.Geometry.PolygonGraph.Delaunay.Delaunay {
	...
	public class Site : ICoord {
		...
		public List<Vector2f> Region(Rectf clippingBounds) {
			...
			if (edgeOrientations == null) {
				...
				if ((new Geom.Polygon(region)).PolyWinding() == Winding.CLOCKWISE) {
					region.Reverse();
				}
			}
			return region;
