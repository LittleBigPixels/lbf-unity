...
namespace LBF.ThirdParties.Delaunator
{
    public class Triangulation
    {
        private static readonly double Epsilon = System.Math.Pow(2, -52);
        ...
        public Triangulation(Vector2[] coords)
        {
            ...
            this._hashSize = (int)System.Math.Ceiling(System.Math.Sqrt(n));
            ...
            for (int i = 0; i < n; i++)
            {
                double d = Distance(cx, cy, coords[i].x, coords[i].y);
                if (d < minDist)
                {
                    i0 = i;
                    minDist = d;
            ...
            double i0x = coords[i0].x;
            double i0y = coords[i0].y;
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                double d = Distance(i0x, i0y, coords[i].x, coords[i].y);
                if (d < minDist && d > 0)
                {
                    i1 = i;
                    minDist = d;
            ...
            double i1x = coords[i1].x;
            double i1y = coords[i1].y;
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                double r = Circumradius(i0x, i0y, i1x, i1y, coords[i].x, coords[i].y);
                if (r < minRadius)
                {
                    i2 = i;
                    minRadius = r;
            ...
            double i2x = coords[i2].x;
            double i2y = coords[i2].y;
            ...
            // for (int i = 0; i < n; i++)
            // {
            //     dists.Add(Distance(coords[i].x, coords[i].y, center.Item1, center.Item2));
            // }
            //

            // sort the points by distance from the seed triangle circumcenter
            ids = ids.AsParallel().OrderBy(i => Distance(coords[i].x, coords[i].y, center.Item1, center.Item2)).ToList();
            ...
            for (int k = 0; k < ids.Count; k++)
            {
                ...
                if (k > 0 && System.Math.Abs(x - xp) <= Epsilon && System.Math.Abs(y - yp) <= Epsilon)
                {
                    continue;
                }
                ...
                int e = start;
                int q = hullNext[e];
                while (!Orient(x, y, coords[e].x, coords[e].y, coords[q].x, coords[q].y))
                {
                    e = q;
                    if (e == start)
                    {
                ...
                n = hullNext[e];
                q = hullNext[n];
                while (Orient(x, y, coords[n].x, coords[n].y, coords[q].x, coords[q].y))
                {
                ...
                {
                    q = hullPrev[e];
                    while (Orient(x, y, coords[q].x, coords[q].y, coords[e].x, coords[e].y))
                    {
                ...
                hullHash.SetSafely(this.HashKey(coords[e].x, coords[e].y), e);
            }
        ...
        private int Legalize(int a)
        {
            ...
            while (true)
            {
                ...
                bool illegal = InCircle(
                    coords[p0].x, coords[p0].y,
                    coords[pr].x, coords[pr].y,
                    coords[pl].x, coords[pl].y,
        ...
        private static ValueTuple<double, double> Circumcenter(double ax, double ay, double bx, double by, double cx,
            double cy)
        {
            double dx = bx - ax;
            double dy = by - ay;
        ...
        private static async Task Quicksort(List<int> ids, List<double> dists, int left, int right, int r = 0)
        {
            if (right - left <= 20)
            {
            ...
            else
            {
                ...
                if (right - i + 1 >= j - left)
                {
                    Quicksort(ids, dists, i, right, r + 1);
                    Quicksort(ids, dists, left, j - 1, r + 1);
                }
                else
                {
                    Quicksort(ids, dists, left, j - 1, r + 1);
        ...
        {
            return (int)System.Math.Floor(PseudoAngle(x - this._cx, y - this._cy) * this._hashSize) % this._hashSize;
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static double PseudoAngle(double dx, double dy)
        {
            double p = dx / (System.Math.Abs(dx) + System.Math.Abs(dy));
